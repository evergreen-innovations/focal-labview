...
! Passed Variables:
!REAL(C_DOUBLE), INTENT(IN)      :: from_SC(*)       ! DATA from the super controller
!REAL(C_DOUBLE), INTENT(INOUT)   :: to_SC(*)         ! DATA to the super controller
...
IF ((LocalVar%iStatus >= 0) .AND. (aviFAIL >= 0))  THEN  ! Only compute control calculations if no error has occurred and we are not on the last time step
    CALL WindSpeedEstimator(avrSWAP, LocalVar, CntrPar, objInst, PerfData, DebugVar)
